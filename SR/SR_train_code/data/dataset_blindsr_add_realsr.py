...
class noiseDataset(data.Dataset):
    def __init__(self, dataset='x2/', size=32,other=False):
        ...
        self.size=size
        self.noise_imgs = sorted(glob.glob(os.path.join(base , '*.png')))
...
class DatasetBlindSR_add_RealSR(data.Dataset):
    '''
    # -----------------------------------------
    ...
    def __init__(self, opt):
        super(DatasetBlindSR_add_RealSR, self).__init__()
        self.opt = opt
        ...
        if self.opt["USE_realsr"]:
            ...
            self.paths_GT, self.sizes_GT = realsr_util.get_image_paths(self.data_type, self.opt['realsr_dataroot_GT']) # realsr HR
            self.paths_LQ, self.sizes_LQ = realsr_util.get_image_paths(self.data_type, self.opt['realsr_dataroot_LQ']) # realsr LR 保证 HR图和LR图相一致
            self.noises = noiseDataset(opt['realsr_noise'], opt['H_size'] / opt['scale'],other=True)
    ...
    def __getitem__(self, index):
        ...
        # realsr
            ...
            if self.opt['phase'] == 'train':
                ...
                self.save_img(img_temp, L_path.replace(self.opt["realsr_dataroot_LQ"], self.opt["data_temp"])[:-4] + "_LR.jpg")
            #########保存中间LR图 end
            ...
            if self.opt['phase'] == 'train':
                img_temp = self.tensor2img(img_L)
                self.save_img(img_temp,L_path.replace(self.opt["realsr_dataroot_LQ"], self.opt["data_temp"])[:-4] + "_LR_add_noise.jpg")
                img_temp = self.tensor2img(img_H)
                self.save_img(img_temp, H_path.replace(self.opt["realsr_dataroot_GT"], self.opt["data_temp"])[:-4] + "_HR.jpg")
            #########保存中间图
