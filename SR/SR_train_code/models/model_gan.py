...
class ModelGAN(ModelBase):
    ...
    def __init__(self, opt):
        ...
        if self.is_train:
            ...
            if self.opt_train['E_decay'] > 0:  # 指数移动平均对模型的参数做平均，以求提高测试指标并增加模型鲁棒。
    ...
    # ----------------------------------------
    # load pre-trained G and D model 加载 模型
    # ----------------------------------------
    def load(self):
    ...
    # ----------------------------------------
    # load optimizerG and optimizerD 加载 优化
    # ----------------------------------------
    ...
    def define_loss(self):  # 获取 损失函数
        ...
        if self.opt_train['G_lossfn_weight'] > 0:
            ...
            print('Do not use pixel loss.')  # 像素损失
            self.G_lossfn = None
    ...
    # ----------------------------------------
    # define scheduler, only "MultiStepLR"  # 定义 调度器
    # ----------------------------------------
    ...
    def optimize_parameters(self, current_step):
        ...
        if self.opt_train['gan_type'] in ['gan', 'lsgan', 'wgan', 'softplusgan']:
            ...
            l_d_real.backward()
            # fake
            pred_d_fake = self.netD(self.E.detach().clone()) # 2) fake data, detach to avoid BP to G 伪造数据，分离 避免 回传到G
            l_d_fake = self.D_lossfn(pred_d_fake, False)
            l_d_fake.backward()
        elif self.opt_train['gan_type'] == 'ragan':
            # real
            pred_d_fake = self.netD(self.E).detach()       # 1) fake data, detach to avoid BP to G 伪造数据，分离 避免 回传到G
    ...
    """
    # ----------------------------------------
    # Information of netG, netD and netF 网络结构信息
    # ----------------------------------------
    """

    # ----------------------------------------
    # print network 打印输出网络信息
    # ----------------------------------------
