...
class ModelPlain(ModelBase):
    ...
    # ----------------------------------------
    # save model / optimizer(optional) 保存 模型 及其 参数
    # ----------------------------------------
    ...
    # ----------------------------------------
    # define scheduler, only "MultiStepLR" 定义调度器
    # ----------------------------------------
    ...
    def optimize_parameters(self, current_step):
        ...
        self.netG_forward()
        G_loss = self.G_lossfn_weight * self.G_lossfn(self.E, self.H) # loss 损失 × loss 损失权重
        G_loss.backward() # loss 回传
        ...
        if G_regularizer_orthstep > 0 and current_step % G_regularizer_orthstep == 0 and current_step % \
            ...
            self.netG.apply(regularizer_orth)  # apply函数会递归地搜索网络内的所有module并把参数表示的函数应用到所有的module上， 深度优先算法
        ...
        if G_regularizer_clipstep > 0 and current_step % G_regularizer_clipstep == 0 and current_step % \
                self.opt['train']['checkpoint_save'] != 0:
            self.netG.apply(regularizer_clip) # 参数 初始化 tcd

        # self.log_dict['G_loss'] = G_loss.item()/self.E.size()[0]  # if `reduction='sum'`
        self.log_dict['G_loss'] = G_loss.item()

        if self.opt_train['E_decay'] > 0:
            self.update_E(self.opt_train['E_decay'])  # 更新参数 指数移动平均对模型的参数做平均，以求提高测试指标并增加模型鲁棒。
